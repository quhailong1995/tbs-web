			...
			for (Filter filter : condition) {
				...
				if (!ObjectUtils.isNullOrEmpty(value)) {
					...
					if (operator == Operator.custom) {
						continue;
					} else if(operator == Operator.eqOrNot){
						String valueStr = (String) value;
						if(valueStr.indexOf("eq") >= 0){
							wrapper.eq(property, valueStr.replace("eq", ""));
						}else if (valueStr.indexOf("ne") >= 0) {
							wrapper.ne(property, valueStr.replace("ne", ""));
						}
					}
					else if (operator == Operator.in) {
						wrapper.in(property, (ArrayList<String>)value);
					} 
