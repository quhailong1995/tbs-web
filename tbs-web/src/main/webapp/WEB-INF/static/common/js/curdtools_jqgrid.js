 ...
 * 检测服务器状态
 */
function testState(title,url,gridId) {
...
//打开对话框(添加修改)
function openDialog2(title,url,gridId,width,height){
...
function cacheOperate(title,url,gridId,width,height) {
	width = '80%';
	height = '750px';
	url="/tbs-web/admin/tbs/dmwhcachemanage/{id}/showCacheOperationPage";
    ...
    openDialog2(title,url,gridId,width, height);
}
 ...
 * @param id//主键字段
 */
function testService(title,url,gridId,width,height) {
	width = '650px';
	height = '750px';
	url="/tbs-web/admin/tbs/dmwhservice/{id}/showTestService";
	var rowsData = $("#"+gridId).jqGrid('getGridParam','selarrrow');
	var multiselect=$("#"+gridId).jqGrid('getGridParam','multiselect');
	var rowData= $("#"+gridId).jqGrid('getGridParam','selrow');
	if(!multiselect)
...
/**
 * 获得查询条件
 * @returns {___anonymous7661_7662}
 */
function getParams(gridId){
	var queryParams = {};
...
function search(gridId) {
    ...
    $("#"+gridId).jqGrid('setGridParam',{  
        datatype:'json',  
        postData:getParams(gridId), //发送数据  
        page:1  
...
/**
 * 设置下拉框内容的方法
 * @param nodeName 下拉框标签的name属性
 * @param url      数据来源的url
 * @param keyName  返回数据中要作为option中value属性的值的key的名字
 * @param valueName返回数据中要作为option中text内容的key的名字
 */
function setSelectContent(nodeName,url,keyName,valueName){
	var selectNode = $("select[name='" + nodeName + "']");
	if (selectNode.find("option").length > 1) {
		return;
	}
	$.post(url, function(dataList) {
		for ( var i in dataList) {
			var data = dataList[i];
			var html = '<option value="'+ data[keyName] + '">' + data[valueName] + '</option>';
