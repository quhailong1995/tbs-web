...
//			});
	...
	var sType = $("#serviceSelect select[name='sType']");
	 ...
	 loadData(service.val(),channel.val(),sType.val());
	...
	$.get(adminPath+"/tbs/dmwhservice/getAllService", function(
		...
		data = eval('(' + data + ')');
		for ( var i in data) {
			service.append('<option value="'+data[i].serviceName+'">'
	...
	$.get(adminPath+"/tbs/dmwhchannel/getAllChannel", function(
		...
		data = eval('(' + data + ')');
		for ( var i in data) {
			channel.append('<option value="'+data[i].channelName+'">'
...
/*		for(var i=0;i<8;i++){
	...
	function loadData(serviceName,channelName,sType){
			$.get(adminPath+"/tbs/statistics/getTradeByService/"+serviceName+"/"+channelName+"/"+sType, function(data) {
		//alert(status);
		data = eval('(' + data + ')');
		//alert(data[09]);
		//lineoption.series[0].data.splice(0,lineoption.series[0].data.length);//清空数组
		if(sType=="hour"){
			...
			$.get(adminPath+"/tbs/statistics/getPreDayTime", function(tdata){
		...
		if(sType=="day"){
			...
			$.get(adminPath+"/tbs/statistics/getPreMonth", function(sdata){
				sdata = eval('(' + sdata + ')');
